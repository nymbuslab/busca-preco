...
- [ ] (P0) **Aguardando domínio do cliente** — quando chegar, preencher [pacote/tunnel/config.yml.exemplo](pacote/tunnel/config.yml.exemplo), executar `cloudflared tunnel login/create/route dns` e renomear pra `config.yml`.
- [ ] (P1) Quando domínio estiver configurado: rodar `pacote\preparar-pacote.ps1 -Zip` na raiz pra gerar `pacote-BuscaPreco.zip` e enviar ao cliente.
- [ ] (P1) Cliente instala via `instalar.bat` (como admin) — registra `BuscaPrecoAPI` + `BuscaPrecoTunnel` como serviços Windows.
- [ ] (P1) Atualizar `VITE_API_URL` no Vercel com o subdomínio final do cliente e fazer redeploy.
